...
function showRecipe(recipe, icon, missing) {
    ...
    if (missing === null) {
        // Surprise mode — no ingredient check was performed, show nothing
    } else if (missing.length === 0) {
        ingredientsHtml = '<p class="has-all">🎉 יש לך הכל!</p>';
    } else {
        var available = recipe.ingredients.filter(function (ing) {
            return !missing.includes(ing);
        });
        var availableHtml = available.length > 0
            ? '<p class="has-ingredients">✅ יש לך: ' + available.join(", ") + '</p>'
            : "";
        ingredientsHtml =
            availableHtml +
            '<p class="missing-msg">⚠️ חסר לך: ' + missing.join(", ") + '</p>';
    }
